...
- `ledger_truth_model.md` — the canonical 25 invariants with full leaves, Phase 2 evolution notes, and interactions. Tiebreaker for "is X legal in the ledger."
...
- `rule-type-core.md` — the rule core's domain layer: Trigger / Condition / Action grammar, pure-core vs. Agent Ladder gate separation, lifecycle states (`proposed` / `active` / `demoted` / `retired`) on an axis separate from `current_rung`, three-state match taxonomy (`primary_match` / `guardrail_match` / `almost_match`), retire-and-create-new logic immutability, four-stage delivery (Ring 0 spec / Ring 1 substrate / Ring 2A workflow evaluators / Ring 2B safety surfaces / Ring 3 learner). Ratified 2026-05-26.
...
- `adr/0023-rule-type-core-substrate.md` — Rule Type Core Ring 1 substrate: `rule_registry` (class-table inheritance) + `rule_track_records` + nine enums + `vendor_rules` drift reconciliation (`bundle_type`, `legal_entity_id` COALESCE-expression unique index, `autonomy_tier` → `rule_autonomy_rung`); three single-writer services defined-but-inert; amends ADR-0017. Ratified 2026-05-26.
- `adr/0024-ring2a-core.md` — Ring 2A-core, first consumer of the ADR-0023 substrate: pure-core evaluator (`core/rules/`), §6.1 capping Agent Ladder gate, `RuleRegistryView` canvas, `rule_evaluation_log` (one-write, append-only) + `rule_evaluation_30d_view` (`security_invoker = true`); four greenfield `services/rules/`; reserves INV-RULE-001 + the `INV-RULE-*` prefix. Net-new substrate; no amendment. Ratified 2026-05-26.
- `adr/0025-ring2a-core-implementation-seams.md` — Ring 2A-core implementation seams consuming the ADR-0024 substrate: pure-core evaluator + predicate registry (3/2/1 specificity, `core/rules/`), capping table + boundary-crossing types (`shared/rules/`, the `agent ↛ core` resolution), Agent Ladder gate + three inert stubs (`agent/policies/agent-ladder/`), five `services/rules/` + the `create_vendor_rule_atomic` RPC, four row-action routes + `rule.*` permissions (count-drift discipline), `RuleRegistryView` Stage 1 canvas. Eleven decisions; five-commit rollout; two small migrations (RPC + permissions seed), no new tables. Net-new seams consuming ADR-0024; no amendment. Ratified 2026-05-26.
- `adr/0026-ring2a-authoring.md` — Ring 2A-authoring, the rule-creation path atop the ADR-0024/0025 substrate: conversational rule-drafting (`draftVendorRule` agent tool + vendor resolution), the Four-Questions `ProposedRuleCard` creation-time render, the `POST /rules` create route + `rule.create` permission + create→approve orchestration, `RuleRegistryView` wiring. Net-new authoring seams consuming ADR-0024/0025; no amendment. Ratified 2026-05-29.
- `adr/0027-ring2b-substrate.md` — Ring 2B branch/condition substrate: normalized `rule_branches` + `rule_conditions` (registry-keyed via the `rule_track_records` pattern — no `org_id`, simple cascade FK; JSONB only for the polymorphic `condition_value`), §5.1 logic-freeze immutability trigger (INV-RULE-004, registered at the Ring 2B implementation arc — hybrid scope: UPDATE+TRUNCATE all-path, DELETE user-path), production `branchSource` replacing the `noBranchSource` no-op, thin Seam-1 shadow call to the shipped `evaluateAndDispatch`, `default_account_id` + vendor-name resolution; new `branch_type` enum + extended SECURITY-DEFINER creation RPC; A1a substrate-only (shadow/diagnostic, no live auto-post), pattern-scope only (temporal/inferential deferred to later workflow ships). Ratified 2026-05-30.
