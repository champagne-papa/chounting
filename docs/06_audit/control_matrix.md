...
- **Code enforcement:** Layer-1 + runtime/structural split. **Layer 1:** `evidence_objects_subject_unique` UNIQUE `(org_id, subject_type, subject_id)` (migration `20240177`) — at most one canonical object per subject, DB-enforced. **Layer 2 (runtime/structural):** the persist-before-marking seam in `apps/web/src/app/api/orgs/[orgId]/review/cases/[caseId]/approve-post/route.ts` (annotated `INV-EVIDENCE-001`) — `evidenceObjectService.persist` (org-scoped subject-ownership guard via `LINKED_ENTITY_TABLE_MAP` + `.eq('org_id', …)`, foreign ≡ missing; then assemble; then idempotent upsert refreshing status + trace only) runs after the ledger write and before `advanceCaseAutomation('committed')`. The committed-marking-path enumeration: `transition()` schema-barred from `committed`; the approve→post route is the **sole surface** bearing `target_state: 'committed'` — **two** sites as of board #4 T4 (the single-invoice path's mark + the multi-invoice N-branch's aggregate guard, INV-WORKFLOW-003), each downstream of a persist seam; no other `advanceCaseAutomation` call site targets `'committed'`; the state RPC has no callers outside `documentCaseService`; the preserved auto-commit composite is unreferenced. (Verified at Wave 6 D5 registration HEAD as nine call sites / one committed-mark; re-verified at T4, 2026-07-11, as fourteen / two.) The **control is the seam (each committed-marking path persists first) + review discipline**, the INV-RULE-003/INV-WORKFLOW-002 runtime/structural sub-type.
...
- **Spec leaf:** [`ledger_truth_model.md#inv-workflow-003`](../02_specs/ledger_truth_model.md#inv-workflow-003--aggregate-committed-marking-a-committed-case-has-all-its-extracted-invoices-posted-layer-2)
- **Test coverage:** `apps/web/tests/integration/reviewApprovePostMultiInvoice.integration.test.ts` (2 tests — **happy N-post**: two matched-vendor α → two bills under the per-invoice keys, both α `posted`, case `committed`; **partial-post + re-approval recovery**: one α postable / one not → `partially_posted`, case HOLDS at `approved`, the postable α posted / the other still `pending`; then the gap resolved → re-approval SKIPS the already-posted α (its `posted_bill_id` byte-identical across the two POSTs — no double-write, no write-once trip) and posts the rest → all α posted → `committed`). The safety-direction guarantee is runtime/structural (see Code enforcement); the tests verify the aggregate guard advances `committed` only when all α are posted, not that a future edit cannot mark `committed` past the guard (the leaf's named residual is the explicit non-coverage).
- **Code enforcement:** Runtime/structural (Layer 2, no Layer-1 half). The aggregate committed guard in the approve→post route's multi-invoice N-branch (`apps/web/src/app/api/orgs/[orgId]/review/cases/[caseId]/approve-post/route.ts`, `postMultiInvoiceCase`, annotated `INV-WORKFLOW-003`): `advanceCaseAutomation('committed')` is reached **only** when `unposted.length === 0` (every α posted this request or already `post_status='posted'`); a partial post returns `partially_posted` with the case held at `approved`, the unposted α flagged by its own NULL `posted_bill_id`. Sibling of INV-EVIDENCE-001 at the same route (both gate the `committed` transition; this is the second of the two committed-marking sites). The **control is the guard (the one aggregate-committed path checks all α first) + review discipline**, the INV-EVIDENCE-001/INV-WORKFLOW-002 runtime/structural sub-type.
- **Non-bypassable from application layer:** Partial — **safety direction only** (`committed ⇒ all-α-posted`; the reverse is unregistered, transiently false in the crash window). Nothing at the DB forces the all-α-posted check: a future edit that marks a multi-invoice case `committed` past the guard compiles cleanly and is caught by review + the test above, not by a DB sentinel — a Layer-1 committed-transition trigger (reject `committed` with any unposted α; passes vacuously for α-less cases) is the **named, deferred** defense-in-depth. `service_role` raw-SQL writes outside the service path are not barred (the standard partial).
...
Expected: 29 registered INV-IDs reached in both directions (board #4 T4
added INV-WORKFLOW-003, symmetric on both sides); the raw output carries
exactly four named exceptions outside the registered set — INV-CHECKPOINT-001 (doc-side; Phase-2 Layer-1b reserved,
