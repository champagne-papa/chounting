...
- Sequencing path: artifact drafting → scope-lock → brief
  ...
  Phase 7
- Per-chunk discipline: acceptance criteria, rollback posture,
  test matrix, and Two Laws verification for each Phase 6.5
  chunk
- Decision-class split: governance-critical decisions land in
  scope-lock; product-discovery micro-decisions land in
...
| T | NO `chat_messages` table exists on disk. The a9f1071 §6.3 sub-shapes (6.3.α / 6.3.β / 6.3.γ) assumed substrate that doesn't exist; actual chat substrate is `agent_sessions.turns` JSONB column (migration 121) with per-turn entries inside the JSONB array (no DB PK at turn grain). Strengthens Sub-Q3 dissolution argument: not only is Flow (b) deferred, but the substrate Sub-Q3 was adjudicating against doesn't exist. | Session A §A.7 + §B.2 + migration 121 + apps/web/src/shared/types/chatTurn.ts |
...
consolidated panel + workspace switches.**

- Remove `MainframeRail.tsx` (14 navigation icons)
- **Detach** DocumentIntakeRail.tsx from
  SplitScreenLayout (component remains on disk as a
  Phase 6.5 chunk 3 deletion target; chunk 1 only removes
  its mount point in the shell layout)
...
PendingDocumentsView + DocumentIntakeRail deletion.**
...
- New `PendingDocumentsView.tsx` canvas view — ports
  ...
  (mostly copy)
- Delete DocumentIntakeRail.tsx component after
  state-machine port to PendingDocumentsView confirms
  green (chunk 1 detached the mount; chunk 3 deletes the
  component)
...
- `docs/03_architecture/phase_plan.md` — phase ownership
  (NOTE: per Session A §A.3, this doc is a Phase-1.1-
  closeout-grain historical snapshot enumerating Phase 1.1 /
  1.2 / 1.3 / "Phase 2 and beyond" only; cannot ratify or
  refute chunk-sequencing on chunk-numbering grounds.
  Useful for shell-architecture ownership confirmation
  only, not phase-sequencing authority)
...
Bridge shell consolidation + Zone 1 consolidated panel +
workspace switches. Per chunk-1-Phase-2 precedent:
brief-drafting → scope-lock → implementation.
...
**Estimated total to Phase 7 start: ~16 sessions** from
current state, with Phase 6.5 taking ~6-9 sessions, Phase 5.1
taking ~1-2 sessions, plus artifact drafting + scope-lock +
brief amendment cycle sessions before chunks fire.
**Estimated total to v1 close: ~18-22 sessions**, assuming
Phase 7 remains within a ~2-6 session extraction/orchestration
envelope. If Phase 7 scopes larger at brief-drafting time,
v1 close moves proportionally. Provisional pending Phase 7
brief-drafting.
...
## 12. CTO sign-off record

### 12.1 Sign-off date and status

**Date**: 2026-05-16
**Status**: Signed off with four requested edits + sign-off
posture per CTO feedback document
**Effective**: This v3 supersedes the v2 proposal at
`docs/09_briefs/phase-6/2026-05-16-cto-proposal-final-document-drop-shell-consolidation.md`
(same `phase-6/` folder). v2 preserved with supersession header
per ADR-0022 §2 supersession discipline at proposal-document
grain.

### 12.2 CTO sign-off posture

Per CTO feedback:

- **Approval of Decisions 1, 2, 3, 5, and 6** (supersession;
  Phase 6.5 framing; Path 2 multi-tab; per-chunk discipline;
  decision-class split).
- **Conditional approval of Decision 4 (sequencing)** with
  session-count language revised to distinguish "to Phase 7
  start" from "to v1 close" (integrated as Edit 2).

### 12.3 Four edits applied (this v3)

1. Executive summary lists six decisions, not four (§1).
2. Phase 7 / session-count estimate language reconciled to
   distinguish "to Phase 7 start" (~16 sessions) from "to v1
   close" (~18-22 sessions, provisional) (§7 Step 10).
3. DocumentIntakeRail removal clarified as two-step:
   chunk 1 detaches mount; chunk 3 deletes component (§5.1).
4. Phase 6.5 chunk 1 renamed "Bridge shell consolidation +
   Zone 1 consolidated panel + workspace switches" to avoid
   "substrate" confusion with dissolved Δ.4 routing
   substrate (§5.1).

### 12.4 Session A preserved-evidence refinements

Two small refinements integrated from Session A's
preserved-evidence closeout (commit 7834a26):

1. `phase_plan.md` annotated as Phase-1.1-closeout-grain
   historical snapshot, not living phase-tracking authority
   (§6.1).
2. Finding T added to Pass 2 findings table: no
   `chat_messages` table exists on disk; strengthens Sub-Q3
   dissolution argument (§2.2).

### 12.5 Likeliest CTO pushbacks (anticipated; preserved for future-session reference)

Per CTO feedback, three pushbacks were named as likeliest:

1. Phase 5.1 insertion before Phase 7. **Resolved**: CTO
   accepts the proposal's framing (Phase 6 retrospective §6
   names Phase 5.1 as carry-forward; veto would require
   retrospective amendment).
2. ~18-22 session count too long. **Resolved**: session-
   count language revised per Edit 2.
3. Multi-tab at v1 (Path 2 vs Path 3). **Resolved**: CTO
   accepts Path 2 (limited multi-tab) with Sub-Q14/15/16/17
   deferrals + Sub-Q19 library choice.
