...
3. **QW-05 trigger over composite-FK.** Trigger reads `chart_of_accounts.org_id` for the inserted `account_id` and `journal_entries.org_id` for the inserted `journal_entry_id`; raises on mismatch. Composite-FK would require denormalizing `org_id` into `journal_lines` — more invasive.
   - **QW-05 trigger fires on BOTH INSERT and UPDATE.** Even though `journal_lines` is made append-only by QW-04 in this same session (so `UPDATE` should never fire in production), the trigger must cover `BEFORE INSERT OR UPDATE`, not `BEFORE INSERT` alone. Rationale: QW-04's immutability trigger fires before QW-05's in PG trigger order, so a malformed `UPDATE` that QW-04 rejects never exercises QW-05 today — but a future schema change that loosens QW-04 (e.g., allowing soft-deletes on lines, adding a `voided` column for some Phase 2 feature) shouldn't silently re-open QW-05's cross-org gap. Belt-and-suspenders defense-in-depth: `BEFORE INSERT OR UPDATE` costs nothing today and survives a future loosening of QW-04. Apply at the migration's `CREATE TRIGGER` statement.
