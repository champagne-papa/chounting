...
5. **`ServiceError` wrapping at `chartOfAccountsService.get()` line 62.** Replace `throw error` with `throw new ServiceError('NOT_FOUND', error.message)` (or appropriate code) so callers see a typed error, not a raw Supabase error. Match the pattern used elsewhere in `src/services/accounting/`.
   - **Agent-tool reachability check (must ship in S25, not Phase 2).** When extending `chartOfAccountsService.get()` input to `{ account_id, org_id }` (option 1 / pattern-consistency choice), audit whether the function is reachable from any agent tool. Run `grep -rn "chartOfAccountsService.get\|chartOfAccountsService\\.get" src/agent/` at Task 2. If a tool calls it, two-line follow-on within this same session: (a) update the corresponding Zod schema in `src/agent/tools/schemas/` to require `org_id` as a top-level input field; (b) verify the orchestrator's org-scoped tool dispatch — `ORG_SCOPED_TOOLS` Set at `src/agent/orchestrator/index.ts:1098` — includes the tool so the orchestrator's pre-Zod `org_id` injection (Site 1) flows through. If the tool is NOT in the Set today, add it. The fix is minimal but must ship in S25 alongside the service change: shipping the service-layer org_id requirement without the schema/Set update would silently break the agent path the moment the tool fires. Phase 2 deferral is not safe because the broken state is between commits within the same arc.
