...
- [disk, must re-ground] **Mailbox may already be partially built**: `ingestionService.handleForwardedMailbox` exists, `20240155_forwarded_mailbox_substrate.sql`, `resolveOrgFromMailboxHash.ts`. So "add mailbox" may be "finish/activate mailbox." **SharePoint** — no grounding; likely net-new (needs a storage-provider seam; storage today = Supabase + Modal OCR sidecar). Deferred next read: `ingestionService` both handlers, the mailbox substrate, the storage-provider layer.
